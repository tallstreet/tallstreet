...
Usage:
  %s [flags]
    ...
    --kind=<string>     Name of the Entity object kind to put in the datastore.
                        (Required)
    --skip=<int>        Number of CSV rows to skip before beginning import.
                        (Useful for resuming after errors)
...
def ImportCSV(filename,
  ...
  written = 0
  try:
    content_gen = create_content_generator(csv_file, batch_size, skip)
    ...
    for num_entities, content in content_gen:
      ...
      try:
        ...
        written += num_entities
      except PostError, e:
        logging.error('An error occurred while importing: %s', e)
        logging.info("Importing stopped on source row %d", skip+written)
        return False
  finally:
    csv_file.close()
  return True
...
def ParseArguments(argv):
  ...
  return (url, filename, cookie, batch_size, kind, skip)
...
def main(argv):
  ...
  url, filename, cookie, batch_size, kind, skip = args
  if ImportCSV(filename, url, cookie, batch_size, kind, skip):
