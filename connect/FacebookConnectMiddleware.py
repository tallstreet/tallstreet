					...
					if(signature_hash == request.COOKIES[API_KEY]):
						...
						if(datetime.fromtimestamp(float(request.COOKIES[API_KEY+'_expires'])) > datetime.now()):
							...
							if user is not None:
								...
								django_user = User.get_by_key_name("user%s" % request.COOKIES[API_KEY + '_user'])
								if not django_user:
									...
									# Create user
									user = User(key_name="user%s" % request.COOKIES[API_KEY + '_user'], username = "%s %s" % (user_info_response[0]['first_name'], user_info_response[0]['last_name']), 
